...
def my_perceptron(x):
    ...
    a = tf.reduce_sum(tf.multiply(i,W))
    out = my_relu(a)
    return i, out
...
def onelayer(X, Y, layersize=10):
    ...
    """
    w = tf.Variable(tf.random_normal([784, layersize], stddev=0.01, seed=1))
    b = tf.Variable(tf.constant(0.01, shape=(layersize,)))

    logits = tf.matmul(X, w) + b
    preds = tf.nn.softmax(logits)

    batch_xentropy = -tf.reduce_sum(tf.multiply(Y, tf.log(preds + 1e-8)), axis=1, keep_dims=True)
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    :return: The following variables should be returned in the following order.
        ...
        batch_loss: The average cross-entropy loss of the batch
    """
    w1 = tf.Variable(tf.random_normal([784, hiddensize], stddev=0.01, seed=1))
    b1 = tf.Variable(tf.constant(0.01, shape=(hiddensize,)))
    w2 = tf.Variable(tf.random_normal([hiddensize, outputsize], stddev=0.01, seed=1))
    b2 = tf.Variable(tf.constant(0.01, shape=(outputsize,)))
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    ## reshape the input to (None, 28, 28, 1)
    ...
    conv2 = tf.layers.conv2d(inputs=conv1,
                            ...
                            kernel_size=filter_shape,
                            padding=padding, activation=tf.nn.relu)
    ## reshape the conv_layer2
    height = conv2.get_shape().as_list()[1]
    width = conv2.get_shape().as_list()[2]
    depth = convlayer_sizes[1]
    reshapesize = height * width * depth
    conv2_flatten = tf.reshape(conv2, shape=[-1, reshapesize])

    w = tf.Variable(tf.random_normal([reshapesize, outputsize], stddev=0.01, seed=1))
    b = tf.Variable(tf.constant(0.01, shape=(outputsize,)))

    logits = tf.matmul(conv2_flatten, w) + b
    preds = tf.nn.softmax(logits)
